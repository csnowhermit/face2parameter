...
def faceparsing_tensor(input, image, face_parse, is_label=False):
    ...
    # 10类：background, face, right eyebrow, left eyebrow, right eye, left eye, nose, upper lip, tooth, down lip
    # 7类：background, face, brow, eye, nose, uplip, downlip
    # weights = torch.tensor([1., 1., 0., 0., 0.9, 0.8, 0.8], dtype=torch.float32).unsqueeze(1).unsqueeze(2)

    # return torch.mul(weights, out), mask_img
    return out, mask_img
...
def main():
    ...
    for i in m_progress:
        ...
        upload_F2_feature, mask_img_upload = faceparsing_tensor(image_F2, image_mask, deeplab, is_label=False)  # 参照
        ...
        # F2_Loss = F.cross_entropy(gen_F2_feature.view(-1, 7, 512*512), upload_F2_feature.view(-1, 512*512).long())    # 这里改用交叉熵损失
        ...
        if i % config.eval_prev_freq == 0:
            ...
            show_img_list = [T.PILToTensor()(img_upload), T.PILToTensor()(T.ToPILImage()(gen_img_copy[0])),
                             T.ToTensor()(mask_img_upload) * 255., T.ToTensor()(mask_img_gen) * 255.]
